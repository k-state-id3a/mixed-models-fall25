...
d3.1 <- read.csv("C:/Users/cdiasda/OneDrive - Kansas State University/Desktop/Doutorado/Laboratory and research/Workshops/Mixed Models Workshop/AartsmaHerbivoreAbundanceOikos.csv")
d3.1 <- as.data.frame(d3.1)
d3.1$Period <- as.factor(d3.1$Period)
d3.1 <- filter(d3.1, Period == "2")
d3 <- d3.1[,c(3, 4, 5, 10)]
d3$Accession <- as.factor(d3$Accession)
d3$Size <- as.factor(d3$Size)
d3$Border <- as.factor(d3$Border)
colnames(d3) <- c("variety", "plot_size", "border", "count")
```
...
## ?? Example 3 - Population count

The following data comes from an experiment conducted to monitor population of two species of pest caterpillars (*Pieris brassica* and *Pieris rapae*) in cabbage (*Brassica oleraceae*). The trials was conducted as a completely randomized with a three way factorial treatment structure 2x2x2, for size of plot (small or large), with or without border (*Brassica nigra*), and variety (less attractive = Badger shipper or more attractice = "Christmas drumhead") ([Aartsma et al. 2020](https://doi.org/10.1111/oik.07484)).
...
-   Why?
    ...
    -   We have counts of events - Number of caterpillars per plot.
...
```{r, message=FALSE, warning=FALSE}
m3 <- glmmTMB(count ~ plot_size*border*variety, family = poisson(link = "log"), data = d3)
summary(m3)
```
...
```{r, message=FALSE, warning=FALSE}
emmeans(m3, ~plot_size*border*variety, type = "response")
```
